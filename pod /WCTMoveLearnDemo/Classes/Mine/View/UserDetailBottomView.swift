...
protocol UserDetailBottomViewDelegate: class {
    /// bottomView 底部按钮的点击
    func bottomView(clicked button: UIButton, bottomTab: Int)
}
...
//    }
    ...
    @objc func bottomTabButtonClicked(button: UIButton) {
        delegate?.bottomView(clicked: button, bottomTab: button.tag)
    }
    ...
    override init(frame: CGRect) {
        ...
        let count = 3;
        let buttonWidth = (screenWidth - CGFloat(count)) / CGFloat(count)
        // 添加按钮
